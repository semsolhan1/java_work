...
BEGIN -- 코드를 실행하는 시작 구간(실행부)
    emp_num := 10; -- 대입 연산자 ' := '
    DBMS_OUTPUT.put_line(emp_num); -- 출력하는 문장을 만드는 함수
...
END; --드레그를 하여 실행하여야 한다. 단축키 F5 블록에 이름이 없어서 (BEGIN, END에)
...
DECLARE
    ...
    v_dep_name VARCHAR2(50); -- 부서명 변수 
                            -- 조회한 결과를 
                            -- INTO를 사용해서 v_dep_name, v_emp_name에 넣겠다.
BEGIN
    SELECT
        e.first_name,
        d.department_name
...
-- 테이블명.컬러명%TYPE을 사용합으로써 타입을 일일히 확인해야 하는 번거로움을 방지할 수 있다.
DECLARE
