...
public class ArrayBasic {
	...
	public static void main(String[] args) {
		...
		*/
		...
		//생성헐 때 배열의 크기를 정해 주셔야 합니다.
		arr = new int[5];
		
		//3. 배열의 초기화 - 배열 내부의 실제 값들을 저장.
...
//		arr[6] = 953;(x)
		...
		//4. 배ㅕㅇㄹ의 총 크기(길이) 확인. -> 배열변수명.length
		System.out.println("arr 배열의 길이: " + arr.length);
		
		//5. 배열에 저장된 값을 참조(사용)하는 법 (인데스를 이용)
		...
		for(int idx=0; idx<=4; idx++) {
			System.out.printf("배열의 %d번째 데이터는: %d\n"
					, idx+1, arr[idx]);
...
//		names = {"홍길동", "김철수", "박영희", "이뽀삐"};  처음일때만 길이 늘리고 싶을때는 새로운 배열을 만들어야한다.
